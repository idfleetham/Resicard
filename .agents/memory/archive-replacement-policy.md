...
The existing development database was baselined through V28; V31 migration `0004` and V32 migration `0005` were then applied normally. From now on, apply outstanding schema changes with the normal migration command. Never run `--baseline` again, never use `drizzle-kit push` on data that must be retained, and do not write replacement DDL when a reviewed migration is available. Production must receive each later migration separately before publishing that version.
