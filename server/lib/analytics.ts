...
/** The label standing in for every band too small to report. */
export const NOT_SHOWN = "not shown";

/** One resident's optional answers. Both are null until they choose to give them. */
...
export interface AgeBandShare { band: string; share: number }
export interface SexShare { value: string; share: number }
export interface Demographics { ageBands: AgeBandShare[]; sex: SexShare[] }

/**
 * Counts one dimension, in the given order, as shares of the residents who
 * answered it. Any value fewer than `minCohort` residents gave is folded into a
 * single "not shown" row: a band of two is two identifiable people, and the
 * merchant knowing which two of their regulars is exactly what this must prevent.
 * Residents who left the question blank are not counted at all, so the shares are
 * of answers given rather than of a denominator nobody can see.
 */
function shares(values: (string | null)[], order: readonly string[], minCohort: number): { value: string; share: number }[] {
  const counts = new Map<string, number>();
  let answered = 0;
  for (const value of values) {
    if (value === null || !order.includes(value)) continue;
  ...
  let withheld = 0;
  const rows: { value: string; share: number }[] = [];
  for (const value of order) {
    ...
    rows.push({ value, share: Math.round((count / answered) * 1000) / 1000 });
  }
  if (withheld > 0) rows.push({ value: NOT_SHOWN, share: Math.round((withheld / answered) * 1000) / 1000 });
  return rows;
}

/**
 * The aggregate demographics of the residents who redeemed at one outlet in the
 * window, or null when there are too few of them for any of it to be anonymous.
 * Rows are one per resident however often they redeemed, so a regular does not
 * count as a crowd.
 */
export function buildDemographics(
  residents: DemographicResident[],
  ageBands: readonly string[],
  sexOptions: readonly string[],
  minCohort: number,
): Demographics | null {
  const unique = new Map<number, DemographicResident>();
  for (const r of residents) if (!unique.has(r.userId)) unique.set(r.userId, r);
  const rows = Array.from(unique.values());
  if (rows.length < minCohort) return null;
  return {
    ageBands: shares(rows.map((r) => r.ageBand), ageBands, minCohort).map(({ value, share }) => ({ band: value, share })),
    sex: shares(rows.map((r) => r.sex), sexOptions, minCohort),
  };
}
